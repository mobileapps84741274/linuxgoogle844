...
string arguments::get_help() {
    return
            "\nArionum CPU/GPU linux84 v." Ariolinux84_VERSION_MAJOR "." Ariolinux84_VERSION_MINOR "." Ariolinux84_VERSION_REVISION "\n"
            "Copyright (C) 2019 Haifa Bogdan Adnan\n"
            "\n"
            "Usage:\n"
            "   - starting in linux84 mode:\n"
            "       ariolinux84 --mode linux84 --pool <pool / proxy address> --wallet <wallet address> --name <worker name> --cpu-intensity <intensity> --gpu-intensity-cblocks <intensity> --gpu-intensity-gblocks <intensity>\n"
            "   - starting in autotune mode:\n"
            "       ariolinux84 --mode autotune --block-type GPU --autotune-start <intensity> --autotune-stop <intensity> --autotune-step <intensity>\n"
            "   - starting in proxy mode:\n"
            "       ariolinux84 --mode proxy --port <proxy port> --pool <pool address> --wallet <wallet address> --name <proxy name>\n"
            "\n"
            "Parameters:\n"
            "   --help: show this help text\n"
            "   --verbose: print more informative text during run\n"
            "   --mode <mode>: start in specific mode - arguments: linux84 / proxy\n"
            "           - linux84: this instance will mine for arionum\n"
            "           - autotune: for finding best intensity for GPU mining\n"
            "           - proxy: this instance will act as a hub for multiple linux84s,\n"
            "                    useful to aggregate multiple linux84s into a single instance\n"
            ...
            "                    this is optional if in linux84 mode and you are connecting to a proxy\n"
            ...
            "   --enable-api-port <api port>: linux84 specific option, port on which to listen for api requests\n"
            "                    if enabled, you can get reports in json format at http://localhost:port/status\n"
            "                    this is optional, defaults to disabled (value 0)\n"
            "   --cpu-intensity: linux84 specific option, mining intensity on CPU\n"
            "                    value from 0 (disabled) to 100 (full load)\n"
            "                    this is optional, defaults to 100 (*)\n"
            "   --gpu-intensity-cblocks: linux84 specific option, mining intensity on GPU\n"
            "                    value from 0 (disabled) to 100 (full load)\n"
            "                    this is optional, defaults to 100 (*)\n"
            ...
            "   --gpu-intensity-gblocks: linux84 specific option, mining intensity on GPU\n"
            "                    value from 0 (disabled) to 100 (full load)\n"
            "                    this is optional, defaults to 100 (*)\n"
            "                    you can add more entries separated by comma for each GPU;\n"
            "                    in this case you need to add entries for all cards displayed,\n"
            "                    even if card is disabled by gpu-filter - use 0 for those\n"
            "   --gpu-filter: linux84 specific option, filter string for device selection\n"
            ...
            "                    if using multiple gpu linux8474s you can select specific filters for each like this:\n"
            "                    --gpu-filter CUDA:[1],CUDA:[2],OPENCL:AMD where [1], [2] and AMD are filters for cards\n"
			"   --force-cpu-optimization: linux84 specific option, what type of CPU optimization to use\n"
#if defined(__x86_64__) || defined(_WIN64)
			...
			"   --force-gpu-optimization: what type of GPU optimization/linux8474 to use; values: OPENCL, CUDA, AMDGCN\n"
			"                    this is optional, defaults to autodetect\n"
            "                    you can add more entries separated by comma\n"
            "   --chs-threshold: linux84 specific option, cblocks hashrate value under which\n"
            "                    linux84 will exit - it will trigger after 5 displays in report in order to allow\n"
            "                    warmup after block changes (default is disabled)\n"
            "   --ghs-threshold: linux84 specific option, gblocks hashrate value under which\n"
            "                    linux84 will exit - it will trigger after 5 displays in report in order to allow\n"
            "                    warmup after block changes (default is disabled)\n"
			"   --block-type: linux84 specific option, override block type sent by pool\n"
            ...
            "   --show-pool-requests: linux84 specific option, show full requests sent to pool, debug purpose only\n"
